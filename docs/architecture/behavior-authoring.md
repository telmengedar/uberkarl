...
| An author writes a script and the facade action silently does nothing | Real and immediate — 9 of 15 intent types are unimplemented (§14, gap G3). Filed as its own task; M5's acceptance is written against implemented actions only. |
...
| **G3** (DiVoid #8052) | **9 of 15 intent types are silently discarded.** `BehaviorRuntime.ApplyIntents` (`:310-337`) handles Hurt/Heal/SetState/MoveToCell/MoveToPosition/MoveBy; `Spawn`, `SetTile`, `Message`, `Despawn`, `SetGraphic`, `ScheduleTimer`, `Teleport`, `SetSpawn`, `SetPhysics` fall to `default: break`. Every one is an advertised facade method that does nothing, with no error. | Bug — the largest authoring trap in the system. |
