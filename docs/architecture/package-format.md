...
- The library lands as a standalone project pair (`src/Uberkarl.Packages` + `tests/Uberkarl.Packages.Tests`). It has no Godot dependency and is unit-tested without a Godot runtime (`dotnet test tests/Uberkarl.Packages.Tests`). It is deliberately **not** added to the Godot-managed `Uberkarl.sln`, so Godot's solution regeneration does not fight it; build the library/tests via their own project paths.
- **Compile-glob guard — APPLIED (A7).** The root Godot `Uberkarl.csproj` (from PR #1, `Godot.NET.Sdk/4.7.1`) globs `**/*.cs`, which would otherwise pull `src/**` and `tests/**` into the game assembly (double-compile + a stray NUnit reference). This branch merges `origin/main` and adds the guard to that csproj: `<Compile Remove="src/**/*.cs" />` and `<Compile Remove="tests/**/*.cs" />`. Verified: `dotnet build Uberkarl.csproj` compiles only the game (`BgmPlayer.cs`) with 0 errors and no `src/`/`tests/` files in its `Compile` item set; `dotnet test tests/Uberkarl.Packages.Tests` stays 31/31. No `ProjectReference` from the game to the library is added yet — the game does not consume packages until Phase 1b; they stay decoupled.
